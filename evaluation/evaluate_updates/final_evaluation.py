...
def evaluate_main_clients_once(
    ...
    cluster_updates = _build_reference_pools(
        clusters,
        main_clients,
        backup_clients,
        main_updates,
    ...
    for cluster_id in clusters:
        ...
        reference_updates = cluster_updates[reference_cluster]

        if reference_cluster not in reference_scale_cache:

            reference_scale_cache[reference_cluster] = compute_reference_scale(
                reference,
                reference_updates,
            )

        median_distance = reference_scale_cache[reference_cluster]
    ...
    available_main_update_clients = set(main_updates.keys())
...
def evaluate_backups_and_replace_once(
    ...
    for cluster_id in clusters:
        ...
        reference_info = round_references.get(cluster_id)

        if not reference_info or reference_info["reference"] is None:
