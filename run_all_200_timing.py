...
Then parses the logs and creates:
    ...
    - summary.txt
    - run_manifest.json
    - timings.csv / timings.txt (algorithm-only runtime)
...
- Baseline defaults in this timing copy: 10 rounds, 1 worker, 0-second cooldown.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--rounds", type=int, default=10, help="Maximum rounds. Default: 10 (baseline timing test)")
    p.add_argument("--workers", type=int, default=1, help="CLIENT_WORKERS. Default: 1 (single-GPU baseline)")
    p.add_argument(
        "--order",
    ...
    p.add_argument(
        ...
        type=int,
        default=0,
        help="Seconds to wait between experiments. Default: 0 for baseline timing",
    )
    p.add_argument(
        "--server-url",
...
def write_timings(timings: Dict[str, float], run_dir: Path) -> None:
    csv_path = run_dir / "timings.csv"
    with csv_path.open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(
            f,
            fieldnames=["algorithm", "elapsed_seconds", "elapsed_hms"],
        )
        writer.writeheader()
        for algo in DEFAULT_ORDER:
            if algo not in timings:
                continue
            sec = float(timings[algo])
            writer.writerow({
                "algorithm": DISPLAY_NAMES[algo],
                "elapsed_seconds": f"{sec:.3f}",
                "elapsed_hms": format_elapsed(sec),
            })

    lines = [
        "Baseline algorithm runtime (run_experiment only)",
        "Registration, cooldown, parsing and plotting are excluded.",
        "",
    ]
    for algo in DEFAULT_ORDER:
        if algo in timings:
            sec = float(timings[algo])
            lines.append(
                f"{DISPLAY_NAMES[algo]}: {format_elapsed(sec)} ({sec:.3f} s)"
            )
...
def main() -> int:
    ...
    try:
        ...
        for idx, algo in enumerate(order):
            ...
            # Measure ONLY the algorithm execution itself.
            # Client registration, cooldown, parsing and plotting are intentionally excluded.
            ...
            print(
                f"\nRUNTIME | {DISPLAY_NAMES[algo]} | "
                f"{format_elapsed(elapsed)} ({elapsed:.3f} seconds)",
                flush=True,
            )
    ...
    banner("ALGORITHM RUNTIME SUMMARY")
    for algo in DEFAULT_ORDER:
        if algo in timings:
            sec = timings[algo]
            print(f"{DISPLAY_NAMES[algo]:<12} {format_elapsed(sec)}  ({sec:.3f} s)")
