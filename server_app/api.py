...
def build_training_package_cache():
    ...
    package = {
        ...
        "global_model": {
            ...
            for name, tensor in server.global_model.state_dict().items()
        },
    ...
    _training_package_bytes = buffer.getvalue()
    _training_package_round = server.current_round
...
def initialize_clustering():
    ...
    return {"status": "clustering initialized", "clusters": server.clusters}
...
def get_model(client_id: int):
    ...
    if client_id not in server.selected_client_ids:
        return {
            "status": "not_selected",
            "selected": False,
            "round": server.current_round,
    ...
    if ( _training_package_bytes is None or _training_package_round != server.current_round):
        raise RuntimeError("Training package cache " "is not initialized for the current round.")

    return Response(content=_training_package_bytes, media_type="application/octet-stream",)
...
async def receive_update(
    ...
    if (round_id != server.current_round ):  
        return {
            "status": "rejected_stale_round",
            "client_id": client_id,
    ...
    update = np.frombuffer(body, dtype=np.float32)
    server.receive_client_update(client_id, update)

    return {"status": "update stored", "client_id": client_id}
...
def aggregate():
    ...
    if not server.main_trust_evaluated:
        ...
        reference_requirements = server.prepare_reference_backup_request()
        ...
        if reference_requirements:
            ...
            return {
                ...
                "stage": "reference",
                "backup_requirements": reference_requirements,
            }

        server.evaluate_main_trust_once()

    # Suspicious-main backups
    replacement_requirements = server.prepare_replacement_backup_request()
    ...
    if replacement_requirements:
        ...
        return {
            "status": "backup_needed",
            "stage": "replacement",
            "backup_requirements": replacement_requirements,
        }
...
def round_selection():
    ...
    fairness = server.get_representation_fairness()

    return {
        "main_clients": server.main_clients,
        "backup_clients": server.backup_clients,
        "representation_fairness": fairness["fairness"],
        "hellinger_distance": fairness["hellinger_distance"],
        "global_distribution": fairness["global_distribution"],
        "selected_distribution": fairness["selected_distribution"],
    }
