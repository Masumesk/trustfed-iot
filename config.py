...
PARTICIPATION_RATIO = _env_float(
    "PARTICIPATION_RATIO",
    0.20
)
...
TRAINING_SEED = _env_int(
    "TRAINING_SEED",
    42
)

SELECTION_SEED = _env_int(
    "SELECTION_SEED",
    42
)
...
ATTACK_TYPE = os.getenv(
    "ATTACK_TYPE",
    None
)

MALICIOUS_RATIO = _env_float(
    "MALICIOUS_RATIO",
    0.20
)

MALICIOUS_SEED = _env_int(
    "MALICIOUS_SEED",
    42
)
...
OPTICS_MIN_SAMPLES = _env_int(
    "OPTICS_MIN_SAMPLES",
    3
)

OPTICS_XI = _env_float(
    "OPTICS_XI",
    0.05
)
...
SELECTION_ALPHA = _env_float(
    "SELECTION_ALPHA",
    0.75
)

BACKUP_RATIO = _env_float(
    "BACKUP_RATIO",
    0.50
)


RANDOM_RATIO = _env_float(
    "RANDOM_RATIO",
    0.30
)
...
if DATASET == "MNIST":
    ...
    LOCAL_EPOCHS = _env_int(
        "LOCAL_EPOCHS",
        2
    )

    LAMBDA_TRUST = _env_float(
        "LAMBDA_TRUST",
        0.80
    )

    TRUST_THRESHOLD = _env_float(
        "TRUST_THRESHOLD",
        0.35
    )
    ...
    TRIM_RATIO = _env_float(
        "TRIM_RATIO",
        0.10
    )

    MIN_REFERENCE_CLIENTS = _env_int(
