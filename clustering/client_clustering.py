...
def client_clustering(
    client_infos, min_samples, xi, min_cluster_size, assignment_threshold
):

    client_ids = sorted(list(client_infos.keys()))  # changed
    client_to_index = {cid: idx for idx, cid in enumerate(client_ids)}  # index
    ...
    medoids = calculate_medoids(G, distance_matrix, client_to_index)
    ...
    medoids = calculate_medoids(G_final, distance_matrix, client_to_index)

    # calculate cluster sample counts
    cluster_samples_counts = calculate_cluster_samples_counts_size(
        G_final, client_infos
    )

    # calculate cluster data shares
    cluster_data_shares = calculate_cluster_samples_share(cluster_samples_counts)
