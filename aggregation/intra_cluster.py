...
def weighted_trimmed_mean(client_updates, client_weights, trim_ratio=0.2):
    ...
    if trim_count <= 0 or 2 * trim_count >= n_clients:
        trim_count = 0
    ...
    weights_all = np.asarray([client_weights[cid] for cid in client_ids])
    ...
    for start in range(0, n_params, chunk_size):
        ...
        kept_indices = order[trim_count : n_clients - trim_count, :]

        kept_values = np.take_along_axis(values, kept_indices, axis=0)

        kept_weights = weights_all[kept_indices]

        kept_weights = kept_weights / kept_weights.sum(axis=0, keepdims=True)

        result[start:end] = np.sum(kept_values * kept_weights, axis=0)
...
def aggregate_clusters(
    ...
    for cluster_id in clusters:
        ...
        accepted = accepted_clients.get(cluster_id, [])
        ...
        for client_id, penalty in accepted:
            ...
            if client_id in main_updates:
                updates[client_id] = main_updates[client_id]

            elif client_id in backup_updates:
                updates[client_id] = backup_updates[client_id]
