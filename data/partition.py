...
def partition_dirichlet(
    ...
    raise RuntimeError("Could not create Dirichlet partition with the requested minimum client size.")


def partition_iid(dataset, num_clients=NUM_CLIENTS, seed=DATA_SEED):
    rng = np.random.default_rng(seed)
    ...
    return [client_indices[i].tolist() for i in range(num_clients)]
